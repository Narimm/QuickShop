				...
				if(info.getAction() == ShopAction.CREATE){
					try{
						...
						if(info.getSignBlock() != null && info.getSignBlock().getType() == Material.AIR && plugin.getConfig().getBoolean("shop.auto-sign")){
							...
							ItemFrame iFrame = (ItemFrame) b.getWorld().spawnEntity(b.getLocation(), EntityType.ITEM_FRAME);
							
							BlockFace[] faces = new BlockFace[]{BlockFace.NORTH, BlockFace.EAST, BlockFace.SOUTH, BlockFace.WEST};
							for(BlockFace face : faces){
								if(face == bf) continue; //This is the sign's location
								...
								/*
								Field handleField = iFrame.getClass().getField("entity");
